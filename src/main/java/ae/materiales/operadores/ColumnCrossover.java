...
public class ColumnCrossover implements CrossoverOperator<IntegerSolution> {
    ...
    public ColumnCrossover(double probability, int numFamilies, int numMaterials) {
    ...
    public List<IntegerSolution> execute(List<IntegerSolution> parents) {
        ...
        // Si no hay crossover: devolvemos una copia del padre como hijo
        ...
        // Cortamos por columnas
        int cut = random.nextInt(numMaterials);

        for (int i = 0; i < numFamilies; i++) {
            for (int j = 0; j < numMaterials; j++) {
                C[i][j] = (j <= cut) ? A[i][j] : B[i][j];
            }
        }
    ...
    // Métodos requeridos por jMetal 6.x
    @Override
