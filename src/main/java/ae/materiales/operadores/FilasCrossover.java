...
public class FilasCrossover implements CrossoverOperator<IntegerSolution> {
    ...
    public FilasCrossover(double probability, int numFamilies, int numMaterials) {
    ...
    public List<IntegerSolution> execute(List<IntegerSolution> parents) {
        ...
        int cut = random.nextInt(numFamilies);

        for (int i = 0; i < numFamilies; i++) {
            for (int j = 0; j < numMaterials; j++) {
                C[i][j] = (i <= cut) ? A[i][j] : B[i][j];
            }
        }
